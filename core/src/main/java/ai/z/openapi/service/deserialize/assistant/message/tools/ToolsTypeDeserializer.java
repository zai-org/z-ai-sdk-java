...
public class ToolsTypeDeserializer extends JsonDeserializer<ToolsType> {
    ...
    public ToolsType deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        // Iterate through classes defined in annotation to determine suitable class based on annotation or static method values
            ...
            if (typeField != null && node.has(typeField.value())) {
                try {
                    ...
                    for (Field field : clazz.getDeclaredFields()) {
                        ...
                        if (node.has(name)) {
                            // Set values based on field type, assuming fields are primitive types or strings
