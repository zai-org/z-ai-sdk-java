 ...
 * Provides endpoints for creating voice clones from audio samples, 
 * deleting existing voice clones, and listing available voice clones.
 */
public interface VoiceCloneApi {

    /**
     * Creates a new voice clone from provided audio sample and parameters.
     * Uses advanced neural voice cloning technology to generate a custom voice model
     * that can synthesize speech matching the characteristics of the provided sample.
     ...
     * Permanently removes the voice model and associated data from the system.
     ...
     * Returns metadata and details for voice clones including voice IDs,
     * names, types, download URLs, and creation timestamps.
     ...
     */
    @GET("voice/list")
    Single<VoiceListResult> listVoices(@Query("voiceType") String voiceType, 
                                       @Query("voiceName") String voiceName);
